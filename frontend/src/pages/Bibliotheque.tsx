...
const Bibliotheque = () => {
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      try {
        // 💡 Utilisation du chemin relatif grâce au Proxy Vite
        const response = await fetch("/api/livrables");
        ...
        <section className="bg-secondary py-16">
          <div className="container">
            ...
            <span className="text-[11px] uppercase tracking-[0.3em] text-muted-foreground">
              {isLoading ? "Chargement en cours..." : `${filtered.length} livrable${filtered.length > 1 ? "s" : ""}`}
            </span>
            </div>

            {isLoading ? (
                <div className="text-center py-24 text-muted-foreground animate-pulse">
                  Connexion sécurisée à la base documentaire...
                </div>
            ) : filtered.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border">
                  {filtered.map((l) => {
                    const MetaIcon = TYPE_META[l.type]?.icon || FileText;
                    const cleanDownloadUrl = l.downloadUrl.includes("http://localhost:3000")
                        ? l.downloadUrl.replace("http://localhost:3000", "")
                        : l.downloadUrl;
                    ...
                    return (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <MetaIcon className="h-7 w-7 text-accent" strokeWidth={1.5} />
